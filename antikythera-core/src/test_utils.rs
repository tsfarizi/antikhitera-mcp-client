...
macro_rules! require_server {
    ($host:expr, $port:expr) => {
        if !$crate::test_utils::is_port_available($host, $port) {
...
macro_rules! require_config {
    () => {
        if !$crate::test_utils::config_available() {
...
macro_rules! require_env {
    ($var:expr) => {
        if !$crate::test_utils::env_var_exists($var) {
...
macro_rules! require_provider {
    ($provider:expr) => {
        if !$crate::test_utils::provider_env_available($provider) {
