...
mod tests {
    ...
    fn prepare_and_commit_plain_text_response() {
        ...
        let telemetry = get_telemetry_snapshot(&session_id).unwrap();
        let telemetry_value: serde_json::Value = serde_json::from_str(&telemetry).unwrap();
        assert!(telemetry_value["counters"]["turns_prepared"].as_u64().unwrap_or(0) >= 1);
        assert!(telemetry_value["counters"]["llm_commits"].as_u64().unwrap_or(0) >= 1);
    }
    ...
    fn stream_chunks_and_drain_events() {
        ...
        append_llm_chunk(&session_id, "\"response\":\"ok\"}", Some("corr-stream")).unwrap();

        let committed = commit_llm_stream(&prepared).unwrap();
        let value: serde_json::Value = serde_json::from_str(&committed).unwrap();
        assert_eq!(value["action"], "final");

        let events = drain_events(&session_id).unwrap();
        let event_values: serde_json::Value = serde_json::from_str(&events).unwrap();
        assert!(event_values.as_array().unwrap().len() >= 4);
    }
}
