...
pub extern "C" fn mcp_security_check_rate_limit(session_id: *const c_char) -> *mut c_char {
    ...
    logger.ffi_call("mcp_security_check_rate_limit", &format!("{{\"session_id\": \"{}\"}}", session_str));

    let guard = RATE_LIMITER.lock().unwrap();
    match guard.as_ref() {
        Some(limiter) => {
            match limiter.check(&session_str) {
                Ok(_) => {
        ...
        None => {
            logger.ffi_error("mcp_security_check_rate_limit", "Rate limiter not initialized");
...
pub extern "C" fn mcp_security_get_usage(session_id: *const c_char) -> *mut c_char {
    ...
    logger.ffi_call("mcp_security_get_usage", &format!("{{\"session_id\": \"{}\"}}", session_str));

    let guard = RATE_LIMITER.lock().unwrap();
    match guard.as_ref() {
        Some(limiter) => {
            match limiter.get_usage(&session_str) {
                Some(usage) => {
...
pub extern "C" fn mcp_security_reset_session(session_id: *const c_char) -> *mut c_char {
    ...
    logger.ffi_call("mcp_security_reset_session", &format!("{{\"session_id\": \"{}\"}}", session_str));
...
pub extern "C" fn mcp_security_remove_session(session_id: *const c_char) -> *mut c_char {
    ...
    logger.ffi_call("mcp_security_remove_session", &format!("{{\"session_id\": \"{}\"}}", session_str));
    ...
    match guard.as_ref() {
        ...
        None => {
            logger.ffi_error("mcp_security_remove_session", "Rate limiter not initialized");
            error_response("Rate limiter not initialized")
        }
    }
}
...
pub extern "C" fn mcp_security_get_rate_limit_config() -> *mut c_char {
    ...
    match guard.as_ref() {
        ...
        None => {
            logger.ffi_error("mcp_security_get_rate_limit_config", "Rate limiter not initialized");
            error_response("Rate limiter not initialized")
        }
    }
}
...
pub extern "C" fn mcp_security_set_rate_limit_config(config_json: *const c_char) -> *mut c_char {
    ...
    match guard.as_mut() {
        ...
        None => {
            logger.ffi_error("mcp_security_set_rate_limit_config", "Rate limiter not initialized");
            error_response("Rate limiter not initialized")
        }
    }
}