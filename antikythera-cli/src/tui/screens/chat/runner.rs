        ...
        if event::poll(timeout)? {
            ...
            match action {
                ...
                InputAction::Command(cmd) => {
                    let show_setup = handle_command(state, &cmd);
                    if show_setup {
                        // Exit chat and open setup menu
                        restore_terminal()?;
                        match crate::tui::screens::run_setup_menu() {
                            Ok(_) => {
                                // Return to caller - they can restart chat if needed
                                return Ok(ChatResult::Exit);
                            }
                            Err(e) => {
                                restore_terminal()?;
                                return Ok(ChatResult::Error(format!("Setup error: {}", e)));
                            }
                        }
                    }
                }
